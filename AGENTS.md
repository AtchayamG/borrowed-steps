...
Before coding report the observable configured session model/effort and whether the serving identity is exposed. Hidden serving identity is not independently verifiable from task text; say unavailable instead of inventing attestation. If the assigned configured model/effort is unavailable or mismatched, write BLOCKED_MODEL; do not silently substitute.
